# Enrich Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `market/backend/enrich_script.py` — a Docker CLI tool that batch-loads products from МойСклад + B&H, auto-suggests categories via Gemini, prompts user to confirm, then fills all characteristics for YM + Ozon + WB and saves to `answers.json`.

**Architecture:** Single file `enrich_script.py` in `market/backend/`. Reuses existing backend modules unchanged. Four async phases: (1) parallel MS+B&H loading, (2) parallel Gemini category suggestion, (3) interactive confirmation, (4) parallel characteristics fill + save.

**Tech Stack:** Python 3.11, asyncio, existing backend modules (moysklad, bh_firecrawl, ai_gemini, spec_extractor, yandex_market, ozon, wb). No new dependencies.
...
|--------|------|----------------|
| Create | `market/backend/enrich_script.py` | All script logic |
| Create | `market/tests/test_enrich_script.py` | Unit tests for pure functions |

---

## Task 1: Script skeleton + pure utility functions + tests

**Files:**
- Create: `market/backend/enrich_script.py`
- Create: `market/tests/test_enrich_script.py`

- [ ] **Step 1: Write the failing tests**

```python
# market/tests/test_enrich_script.py
import sys, os
...
```bash
docker exec market-backend-1 python -m pytest tests/test_enrich_script.py -v 2>&1 | head -30
```

Expected: `ModuleNotFoundError: No module named 'enrich_script'`

- [ ] **Step 3: Create `market/backend/enrich_script.py` with tested functions**
...
def merge_answers(existing: dict, new: dict) -> dict:
    ...
    for k, v in new.items():
        ...
        if cur in (None, "", {}, []):
            ...
            inner = dict(v)           # start with new
            inner.update(cur)         # existing wins
            result[k] = inner
...
```bash
docker exec market-backend-1 python -m pytest tests/test_enrich_script.py -v
```
...
```bash
git add market/backend/enrich_script.py market/tests/test_enrich_script.py
git commit -m "feat: enrich_script skeleton with utilities and tests"
```

---

## Task 2: Phase 1 — Parallel data loading

**Files:**
- Modify: `market/backend/enrich_script.py` — add `load_all_data()`

- [ ] **Step 1: Add `load_all_data` to enrich_script.py** (append after utility section)
...
- [ ] **Step 2: Smoke-test Phase 1 in Docker with one product code**

```bash
docker exec market-backend-1 python -c "
import asyncio
from enrich_script import load_all_data
items = asyncio.run(load_all_data(['18385']))
item = items[0]
print('ok:', item['ok'])
print('name:', item.get('product', {}).get('name', '—'))
print('bh_found:', item.get('bh_found'))
print('bh_specs_count:', len(item.get('bh_specs', {})))
"
```

Expected output (names/counts will vary):
```
Загружаем данные для 1 товаров...
  МС: ✓ 18385
  B&H: ✓ 18385
  Итого: МС ✓1/1  B&H ✓1/1
ok: True
name: SmallRig ...
bh_found: True
bh_specs_count: 12
...
```bash
git add market/backend/enrich_script.py
git commit -m "feat: enrich_script Phase 1 parallel data loading"
```

---

## Task 3: Phase 2 — AI category suggestion + table display

**Files:**
- Modify: `market/backend/enrich_script.py` — add `suggest_categories()`, `build_table_rows()`, `print_category_table()`

- [ ] **Step 1: Add these three functions to enrich_script.py**
...
- [ ] **Step 2: Smoke-test table display in Docker**

```bash
docker exec market-backend-1 python -c "
import asyncio
from enrich_script import load_all_data, suggest_categories, build_table_rows, print_category_table
async def run():
    items = await load_all_data(['18385'])
...
Expected: table printed with category name, confidence, badge for YM/Ozon/WB.

- [ ] **Step 3: Commit**

```bash
git add market/backend/enrich_script.py
git commit -m "feat: enrich_script Phase 2 AI category suggestion + table"
```

---

## Task 4: Phase 3 — Interactive category confirmation

**Files:**
- Modify: `market/backend/enrich_script.py` — add `_show_leaf_categories()`, `confirm_categories()`

- [ ] **Step 1: Add functions to enrich_script.py**
...
```bash
git add market/backend/enrich_script.py
git commit -m "feat: enrich_script Phase 3 interactive category confirmation"
```

---

## Task 5: Phase 4 — Characteristics fill + save + print report

**Files:**
- Modify: `market/backend/enrich_script.py` — add `_fill_marketplace()`, `fill_and_save()`, `print_report()`
...
async def _fill_marketplace(
    ...
    """
    Apply 3 enrichment layers and return merged params dict.

    Layer 2 (rules): deterministic, based on product type flags.
    Layer 3 (Gemini): fills the remaining attributes.
    Rules win over AI on conflict (same logic as ai_enrich_params endpoint).
    """
...
async def fill_and_save(items: list[dict], confirmed: dict[str, dict]) -> list[dict]:
    ...
    Returns list of result dicts:
      {code, name, ym_filled, oz_filled, wb_filled, bh_found, saved}
    """
    ...
    async def _process_one(item: dict) -> dict:
        ...
        # Start building the answers entry
        new_ans: dict = {
            "brand":       (ai.get("brand") or product.get("brand", "") or
                            (product.get("attributes") or {}).get("Бренд", "")),
...
```bash
git add market/backend/enrich_script.py
git commit -m "feat: enrich_script Phase 4 fill characteristics and save"
```

---

## Task 6: main() orchestration + end-to-end test

**Files:**
- Modify: `market/backend/enrich_script.py` — add `get_input_codes()`, `main()`

- [ ] **Step 1: Add `get_input_codes()` and `main()` at end of enrich_script.py**
...
- [ ] **Step 2: Run full end-to-end test with 1 product in Docker**

```bash
docker exec -it market-backend-1 python enrich_script.py 18385
```

Walk through the full flow:
1. Watch Phase 1 output (MS ✓, B&H ✓/✗)
2. Watch Phase 2 Gemini output
3. Table is displayed with suggested categories
4. Press Enter to accept
5. Watch Phase 4 fill output
6. Verify report shows filled counts > 0

- [ ] **Step 3: Verify answers.json was written**
...
print('ozon_category_key:', a.get('ozon_category_key'))
print('wb_category_key:', a.get('wb_category_key'))
print('ym params count:', len(a.get('params_values', {})))
print('ozon attrs count:', len(a.get('ozon_attrs', {})))
print('wb chars count:', len(a.get('wb_chars', {})))
"
```

Expected: brand, category, and at least some attrs filled.
...
- [ ] **Step 5: Run with all 10 target products**

```bash
docker exec -it market-backend-1 python enrich_script.py 18385 18389 17543 17683 18369 18370 03191 18368 18386 18387
```

Review the category table carefully before pressing Enter. Use correction commands if any category looks wrong.

- [ ] **Step 6: Commit final version**

```bash
git add market/backend/enrich_script.py market/tests/test_enrich_script.py
git commit -m "feat: enrich_script complete — batch enrichment with interactive category confirmation"
```
...
**Spec coverage check:**
- ✓ All 3 marketplaces: YM, Ozon, WB
- ✓ Parallel data loading (asyncio.gather in load_all_data, suggest_categories, fill_and_save)
- ✓ B&H not found → continues without specs
- ✓ Interactive confirmation (confirm_categories)
- ✓ 3-layer fill: rules (layer 2) + Gemini (layer 3)
- ✓ save to answers.json + merge without overwrite
- ✓ Console report
- ✓ Run via Docker
- ✓ Existing answers.json values not overwritten (merge_answers)

**Type consistency:**
- `_parse_correction` returns `Optional[tuple[str, str, str]]` — used identically in Task 1 (test) and Task 4 (confirm_categories)
- `merge_answers(existing: dict, new: dict) -> dict` — same signature in test and usage
- `_fill_marketplace(product, mp, attrs, bh_specs, extracted) -> dict` — called identically in fill_and_save
- `items_by_code: dict[str, dict]` passed to `confirm_categories` — required for product name lookup in interactive mode
